...
ImU32 ImHash(const void* data, int data_size, ImU32 seed)
{
    ...
    if (!crc32_lut[1])
    {
        const ImU32 polynomial = 0x82F63B78;
    ...
    if (data_size > 0)
    {
        // Known size
        while (data_size--)
            crc = has_sse42 ? _mm_crc32_u8(crc, *current++) : (crc >> 8) ^ crc32_lut[(crc & 0xFF) ^ *current++];
    }
    else
    {
        ...
        while (unsigned char c = *current++)
        {
            ...
            if (c == '#' && current[0] == '#' && current[1] == '#')
                crc = seed;
            crc = has_sse42 ? _mm_crc32_u8(crc, c) : (crc >> 8) ^ crc32_lut[(crc & 0xFF) ^ c];
        }
    }
    return ~crc;
}
